...
const largeNetQuantityRE = /\(x([0-9,]+)\)/
const fishCaughtItemRE = /<br\/>([^<]*?)\s*(?:\(x([0-9,]+)\))?\s*($|<)/
...
const parseNetResults = (page, url) => {
    ...
    for (const image of dom.querySelectorAll("img")) {
        results.items.push({
            ...
            quantity: results.largeNet ? parseNumberString(image.nextSibling.nodeValue.match(largeNetQuantityRE)[1]) : 1
        })
    }
    return results
}

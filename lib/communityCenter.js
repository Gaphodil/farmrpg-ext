...
class CommunityCenterDB {
  ...
  /**
   * Fetch data for an community center mission.
   * @param {string} date
   */
  async get(date) {
 ...
 */
 const parseCommunityCenter = (dom, url) => {
  ...
  const [goal, reward] = Array.from(todayCard.querySelectorAll(".col-50")).map(elm => {
    ...
    return {
      item: match[1],
      quantity: parseNumberString(match[2]),
    }
  })
  const progress = parseNumberString(todayCard.textContent.match(progressRE)[1])
  comm.missions.push({
    date: today,
    goalItem: goal.item,
  ...
  for (const elm of pastCard.querySelectorAll("li")) {
    ...
    comm.missions.push({
      ...
      goalQuantity: parseNumberString(goalMatch[2]),
      rewardItem: rewardMatch[1],
      rewardQuantity: parseNumberString(rewardMatch[2]),
      progress: parseNumberString(progressMatch[1]),
    })
  }
  ...
  if (match) {
    comm.contributed = parseNumberString(match[1])
  }

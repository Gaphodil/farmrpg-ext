...
class ItemDB {
    ...
    async getByID(id) {
        const item = await this.db.get("items", String(id))
        return item ? new Item(item) : item
    }
}
...
const parseDuration = (str) => {
    ...
    if (match) {
        const days = parseNumberString(match[1] || "0")
        const hours = parseNumberString(match[2] || "0")
        const minutes = parseNumberString(match[3] || "0")
...
const parseItemInfo = (dom, parsedUrl) => {
    ...
    for (const heading of dom.querySelectorAll(".content-block-title")) {
        if (heading.textContent.includes("Item Details")) {
    ...
    for (const row of cardElm.querySelectorAll(".item-content")) {
        ...
        case "Growth Time":
            ...
            break
        case "Craftable":
            // update changed this to "current inventory / required amount"
            item.craftPrice = parseNumberString(value.match(craftNumberRE)[1])
            break
        case "Go to the Locksmith":
            item.openable = true
            item.opensTo = []
            break
        }
    }
    // Then look for a recipe or locksmith section.
    let listElm = undefined
    let doRecipe = false;
    for (const heading of dom.querySelectorAll(".content-block-title")) {
        if (heading.textContent.includes("Crafting Recipe")) {
            doRecipe = true;
            listElm = heading.nextElementSibling.nextElementSibling
            item.recipe = []
            break
        }
        if (heading.textContent.includes("Item Contents")) {
            listElm = heading.nextElementSibling.nextElementSibling
            if (!item.openable) {
                item.openable = true
                item.opensTo = []
    ...
    if (listElm) {
        for (const row of listElm.querySelectorAll("li")) {
            ...
            // discard "Gold" from locksmith
            if (ingredientName === "Gold") {
                continue
            }
            if (doRecipe) {
                ...
                item.opensTo.push({id: ingredientId, name: ingredientName, quantity: parseNumberString(ingredientCount)})
            }
        }
    }
    // If openable and no chest, this must be a grab bag
    if (item.openable && !item.opensTo.length) {
        item.opensTo = fetchGrabBagItems(item.id)
    }
    return item
}
...
const visitItemInfo = async (state, dom, parsedUrl) => {
    ...
    // locksmith item handling
    if (item.openable) {
        state.lastView = "locksmithItem"
        state.lastItem = item.id
    }
}
...
const clickItem = async (state, eventType, eventArg, msg) => {
    ...
    if (quickAction !== undefined && !msg.shift) {
        ...
        const quantity = Math.min(
            Math.max(
                (state.player.inventory[targetItem.name] || 1)
                    - (quickAction.keep === undefined ? 0
                        : quickAction.keep >= 1 ? quickAction.keep
                        : state.player.maxInventory * quickAction.keep),
                0),
            state.player.maxInventory
        )
        ...
        switch (quickAction.action) {
            case "sell":
                ...
                state.player.inventory[targetItem.name] -= quantity
                break
            case "mail":

...
const parseExploreResults = (page, url) => {
    ...
    results.mult = parseNumberString(parsedUrl.searchParams.get("mult") || "1")
    // Parse the name and items from the HTML.
    const parser = new DOMParser()
    const dom = parser.parseFromString(page, "text/html")
    if (results.cider) {
        // Parse cider results which work diffently from the other two.
        const headerMatch = page.match(/explored <strong>([0-9,]+)x<\/strong>.*<strong>([0-9,]+)<\/strong>\s+stamina.*<strong>([0-9,]+)<\/strong>\s+items\s+found/)
        if (!headerMatch) {
            throw "Unable to parse stamina from cider results"
        }
        results.explores = parseNumberString(headerMatch[1])
        results.stamina = parseNumberString(headerMatch[2])
        // We don't need it per se but useful for verification of the per-item parsing
        results.totalItems = parseNumberString(headerMatch[3])
    } else if (page.startsWith("Lemonade helped you find:")) {
        results.lemonade = true
    } else if (page.startsWith("Arnold Palmer helped you find:")) {
        results.arnoldPalmer = true
    } else {
        ...
        if (numberRE.test(strong.textContent)) {
            // Multi-explore.
            results.stamina = parseNumberString(strong.textContent)
        }
    }
    for (const image of dom.querySelectorAll("img")) {
        let quantity = 1
        if (results.cider || results.arnoldPalmer) {
            // For cider we need to pull the (xN) after the image.
            const text = image.nextSibling
            if (text.nodeName !== "#text") {
                console.error("error parsing cider/palmer item quantity, got next element", text)
                continue
            }
            const quantityMatch = text.nodeValue.match(/\(x([0-9,]+)\)/)
            if (!quantityMatch) {
                console.error("error parsing cider/palmer item quantity, no match", text)
                continue
            }
            quantity = parseNumberString(quantityMatch[1])
        }
        results.items.push({
...
export const setupExplore = state => {
    state.addWorkerHandler("explore", visitExploreResults)
    // Lemonade.
    state.addWorkerHandler("drinklm", visitExploreResults)
    state.addPageHandler("area", visitArea)

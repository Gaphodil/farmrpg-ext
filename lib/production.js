...
const renderProductionRow = async (state, itemName, quantity, link) => {
    ...
    const overflow = Math.max((state.player.inventory[itemName] || 0) + quantity - state.player.maxInventory, 0)
            ...
            <div class="item-content">
                ...
                <div class="item-inner">
                    ...
                    <div class="item-after">
                        <span class="farmrpg-ext-production-overflow">${overflow === 0 ? "" : overflow}</span>
                        ${quantity - overflow}
                    </div>
                </div>
            </div>
...
const renderDaily = async state => {
    ...
    if (state.player.orchard !== null) {
        ...
        rows.push(await renderProductionRow(state, "Apple", state.player.orchard.Apple, orchardLink))
        rows.push(await renderProductionRow(state, "Orange", state.player.orchard.Orange, orchardLink))
        rows.push(await renderProductionRow(state, "Lemon", state.player.orchard.Lemon, orchardLink))
    }
    // Vineyard.
    if (state.player.vineyard !== null) {
        rows.push(await renderProductionRow(state, "Grapes", state.player.vineyard, `vineyard.php?id=${state.player.farmID}`))
    }
